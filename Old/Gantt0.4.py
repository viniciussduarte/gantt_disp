...
OUTPUT_FILE = 'alocacao_equipe_ferias_por_matricula.png'
...
def main():
    ...
    if equipe_df is not None:
        ...
        print("\nEquipe (ordenada por Disciplina, Função e Projeto):")
        for _, row in equipe_df.sort_values(by=['Disciplina', 'Função', 'Projeto', 'Matrícula']).iterrows():
            print(f"{row['Disciplina']} - {row['Função']} - {row['Projeto']}: {row['Matrícula']} - {row['Nome']}")
    else:
